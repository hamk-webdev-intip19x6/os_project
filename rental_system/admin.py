...
class RentedWorkAdmin(admin.ModelAdmin):
    list_display = ('user', 'rented_work', 'rent_date',
                    'return_date', 'date_returned', 'returned')
    search_fields = ['rented_work__title', 'user__username']
    list_filter = ['rented_work__type', 'returned']


class WorkAdmin(admin.ModelAdmin):
    list_display = ('title', 'desc', 'type', 'pub_date', 'available')
    list_filter = ['pub_date', 'type',
                   'publishers__publisher', 'genres__genre']
    search_fields = ['title']
